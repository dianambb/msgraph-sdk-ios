...
- (NSArray*) charts
{
    ...
    if ([charts isKindOfClass:[NSArray class]]){
        for (id workbookChart in charts){
            [chartsResult addObject:[[MSGraphWorkbookChart alloc] initWithDictionary: workbookChart]];
        }
    }
...
- (NSArray*) pivotTables
{
    ...
    if ([pivotTables isKindOfClass:[NSArray class]]){
        for (id workbookPivotTable in pivotTables){
            [pivotTablesResult addObject:[[MSGraphWorkbookPivotTable alloc] initWithDictionary: workbookPivotTable]];
        }
    }
