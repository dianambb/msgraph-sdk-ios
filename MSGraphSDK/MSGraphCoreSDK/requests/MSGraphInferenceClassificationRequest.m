...
@implementation MSGraphInferenceClassificationRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphInferenceClassification *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphInferenceClassification alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphInferenceClassification *)inferenceClassification
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[inferenceClassification dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:inferenceClassification]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphInferenceClassification alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
