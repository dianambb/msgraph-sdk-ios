...
@implementation MSGraphTaskRequest
...
- (NSMutableURLRequest *)update:(MSGraphTask *)task
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[task dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:task]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphTask alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
