...
@implementation MSGraphEventMessageRequestEntityRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphEventMessageRequestEntity *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEventMessageRequestEntity alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphEventMessageRequestEntity *)eventMessageRequestEntity
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[eventMessageRequestEntity dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:eventMessageRequestEntity]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphEventMessageRequestEntity alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
