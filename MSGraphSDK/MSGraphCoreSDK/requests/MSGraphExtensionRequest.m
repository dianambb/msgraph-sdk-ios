...
@implementation MSGraphExtensionRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphExtension *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphExtension alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphExtension *)extension
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[extension dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:extension]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphExtension alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
