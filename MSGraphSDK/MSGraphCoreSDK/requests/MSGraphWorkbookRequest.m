...
@implementation MSGraphWorkbookRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphWorkbook *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbook alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphWorkbook *)workbook
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[workbook dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:workbook]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphWorkbook alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
