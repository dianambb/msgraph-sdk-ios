...
@implementation MSGraphPlanDetailsRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphPlanDetails *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlanDetails alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphPlanDetails *)planDetails
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[planDetails dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:planDetails]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlanDetails alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
