...
@implementation MSGraphSectionRequest
...
- (NSMutableURLRequest *)update:(MSGraphSection *)section
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[section dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:section]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSection alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
