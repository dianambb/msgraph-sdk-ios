...
@implementation MSGraphApplicationRequest
...
- (NSMutableURLRequest *)update:(MSGraphApplication *)application
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[application dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:application]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphApplication alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
