...
@implementation MSGraphOpenTypeExtensionRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphOpenTypeExtension *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOpenTypeExtension alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphOpenTypeExtension *)openTypeExtension
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[openTypeExtension dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:openTypeExtension]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphOpenTypeExtension alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
