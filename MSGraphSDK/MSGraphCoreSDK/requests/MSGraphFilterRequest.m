...
@implementation MSGraphFilterRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphFilter *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphFilter alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphFilter *)filter
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[filter dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:filter]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphFilter alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
