...
@implementation MSGraphPageRequest
...
- (NSMutableURLRequest *)update:(MSGraphPage *)page
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[page dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:page]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPage alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
