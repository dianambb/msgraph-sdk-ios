...
@implementation MSGraphPlanTaskBoardRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphPlanTaskBoard *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlanTaskBoard alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphPlanTaskBoard *)planTaskBoard
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[planTaskBoard dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:planTaskBoard]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPlanTaskBoard alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
