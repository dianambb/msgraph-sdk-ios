...
@implementation MSGraphIdentityRiskEventRequest
...
- (NSMutableURLRequest *)update:(MSGraphIdentityRiskEvent *)identityRiskEvent
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[identityRiskEvent dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:identityRiskEvent]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphIdentityRiskEvent alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
