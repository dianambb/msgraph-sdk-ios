...
@implementation MSGraphItemAttachmentRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphItemAttachment *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphItemAttachment alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphItemAttachment *)itemAttachment
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[itemAttachment dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:itemAttachment]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphItemAttachment alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
