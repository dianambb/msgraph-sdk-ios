...
@implementation MSGraphDirectoryRoleRequest
...
- (NSMutableURLRequest *)update:(MSGraphDirectoryRole *)directoryRole
{    
    NSData *body = [NSJSONSerialization dataWithJSONObject:[directoryRole dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:directoryRole]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphDirectoryRole alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
