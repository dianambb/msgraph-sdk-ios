...
@implementation MSGraphPrivilegedRoleAssignmentRequest
...
- (NSMutableURLRequest *)update:(MSGraphPrivilegedRoleAssignment *)privilegedRoleAssignment
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[privilegedRoleAssignment dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:privilegedRoleAssignment]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPrivilegedRoleAssignment alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
