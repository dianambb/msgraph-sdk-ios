...
@implementation MSGraphPrivilegedRoleRequest
...
- (NSMutableURLRequest *)update:(MSGraphPrivilegedRole *)privilegedRole
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[privilegedRole dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:privilegedRole]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPrivilegedRole alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
