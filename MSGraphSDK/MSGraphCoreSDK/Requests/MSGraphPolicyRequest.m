...
@implementation MSGraphPolicyRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphPolicy *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPolicy alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphPolicy *)policy
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[policy dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:policy]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphPolicy alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
