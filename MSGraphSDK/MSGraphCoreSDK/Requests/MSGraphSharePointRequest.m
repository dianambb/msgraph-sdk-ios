...
@implementation MSGraphSharePointRequest
...
- (MSURLSessionDataTask *)getWithCompletion:(void (^)(MSGraphSharePoint *response, NSError *error))completionHandler
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self get]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSharePoint alloc] initWithDictionary:response];
                                        }
...
- (NSMutableURLRequest *)update:(MSGraphSharePoint *)sharePoint
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[sharePoint dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:sharePoint]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphSharePoint alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
