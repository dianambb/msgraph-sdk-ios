...
@implementation MSGraphConnectorRequest
...
- (NSMutableURLRequest *)update:(MSGraphConnector *)connector
{
    NSData *body = [NSJSONSerialization dataWithJSONObject:[connector dictionaryFromItem] options:0 error:nil];
...
{
    MSURLSessionDataTask *sessionDataTask = [self taskWithRequest:[self update:connector]
                                odObjectWithDictionary:^(NSDictionary *response){
                                            return [[MSGraphConnector alloc] initWithDictionary:response];
                                        }
                                              completion:completionHandler];
